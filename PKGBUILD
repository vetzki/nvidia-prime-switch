...
depends=('xf86-video-intel' 'python' 'xorg-xrandr')
...
source=('prime-switch.py' 'prime-switch-conf.json' '99-xrandr-prime.sh' 'intel.conf' 'nvidia.conf' 'intel-modesetting.conf' '999-nvidia-gpu-power.rules' 'nvidia-prime-displaymanager.hook' 'prime-switch-systray.py' 'prime-switch-systray.desktop')
sha256sums=(
...
if [ $with_udev == 1 ];
  then
    backup=('etc/X11/mhwd.d/intel.conf' 'etc/X11/mhwd.d/intel-modesetting.conf' 'etc/X11/mhwd.d/nvidia.conf' 'etc/X11/xinit/xinitrc.d/99-xrandr-prime.sh' 'etc/udev/rules.d/999-nvidia-gpu-power.rules')
  else
